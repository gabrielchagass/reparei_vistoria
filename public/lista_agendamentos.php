...
<div class="container">
    <h2>Autorização de entraga de chaves</h2>
           ...
           <tr>
            ...
            <?php
                ...
                while ($row = $result->fetch_assoc()) {
                    $nome_at=explode("@",$row['email']);
                    $nome_at=$nome_at[0];
                    $agendamento_id_at=$row['agendamento_id'];
                    $cod='';
                    $chve_at='';
                    $sqlb = "SELECT * FROM agendamentos WHERE id = $agendamento_id_at";
                    $resultb = $conn->query($sqlb);
                    while ($rowb = $resultb->fetch_assoc()) {
                        $cod=$rowb['contrato_cod'];
                        $chve_at=$rowb['imovel_endereco'] . ', '. $rowb['imovel_numero'].' '.$rowb['imovel_complemento'] ;
                        echo '<tr><td>'.$nome_at.'</td>';
                        echo '<td>'.$cod.'</td>';
                        echo '<td>'.$chve_at.'</td></tr>';
                    }
                }
...
<div class="container">
    <h2>Imóveis Indisponiveis:</h2>
    <table class="table table-striped">
            <?php
            
                $sql = "SELECT * FROM agendamentos WHERE solicitacao_bloqueada = 1 AND deleted_at IS NULL AND data_fim IS NULL ".$filtro." ORDER BY prazo_fim ASC";
                $result = $conn->query($sql);
                ...
                if ($result->num_rows > 0) {
                    ...
                    echo '<th><div>Prazo</div><div><small>(Fixado somente após imóvel estar disponivel)</small></div></th>';
                    echo '<th>Ações</th>';
                    echo '</tr>';
                    echo '</thead>';
                    echo '<tbody>';
                    ...
                    while ($row = $result->fetch_assoc()) {
                        //verifica se pode arrastar e soltar
                        $dragable='';
                        $data_liberacao = date('Y-m-d H:i:s');
                        $displayprazo=calcularHorarioConclusao($data_liberacao, 20);
                        $class_edit_link='edit-link';
                        $allow_button='<button type="button"  onclick="exibirModalliberacao(' . $row['id'] . ')" class="btn btn-success liberar-agendamento" data-id="'.$row['id'].'">Liberar</button>';
                        $cancel_button='<button type="button"  onclick="exibirModal(' . $row['id'] . ')" class="btn btn-danger cancelar-agendamento" data-id="'.$row['id'].'">Cancelar</button>';
    ...
    <div class="container">
        <h2>
            ...
            <select class="form-control" id="list_status" name="list_status" onchange="aplicarFiltro(this.value)">
                <option value="1">Aguardando Vistoria</option>
                <option value="4">Vistoria Concluida</option>
            </select>   
        </h2>

        <?php
        // Consulta SQL para selecionar os registros da tabela "agendamentos" com data_fim nulo
        $sql = "SELECT * FROM agendamentos WHERE solicitacao_bloqueada = 0 AND  deleted_at IS NULL AND data_fim IS NULL ".$filtro." ORDER BY prazo_fim ASC";
        $result = $conn->query($sql);
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                $dragable='';
                $proxima_txt='';
                $displayprazo=date('d/m/Y H:i', strtotime($row['prazo_fim'])) ;
                $class_edit_link='edit-link';
                $cancel_button='<button type="button"  onclick="exibirModal(' . $row['id'] . ')" class="btn btn-danger cancelar-agendamento" data-id="'.$row['id'].'">Cancelar</button>';
                if(in_array($row['id'], $em_andamento)){
                    $displayprazo='Vistoriando...';
                    $class_edit_link='not_editable';
                    $cancel_button='';
                }else if($row['id'] == $proxima){
                    $dragable='';
                    $proxima_txt='<div><small>(Próxima da fila)</small></div>';
                }else if($row['tipo_vistoria_id']==1){
